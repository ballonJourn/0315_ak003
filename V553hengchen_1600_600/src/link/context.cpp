...
#define ENABLE_LINK_START_HANDELER 1 // 1 互联启动、停止、重启放入handler中运行
...
static void _mix_mode_player_put_frame(bool phoning, uint8_t *data, uint32_t size) {
	if (phoning) {
		mpi::AudioSource::multiply((short*) data, sizeof(data)/sizeof(short), 1.02f);
...
#if ENABLE_LINK_START_HANDELER
void start_lylink() {
	_s_handler.remove_messages(E_START_LYLINK);
	_s_handler.send_message(E_START_LYLINK);
}
void stop_lylink() {
	_s_handler.remove_messages(E_STOP_LYLINK);
	_s_handler.send_message(E_STOP_LYLINK);
}
void restart_lylink() {
	_s_handler.remove_messages(E_RESTART_LYLINK);
	_s_handler.send_message(E_RESTART_LYLINK);
}
static void _message_handler(const fy::message *msg, const void *user_data) {
